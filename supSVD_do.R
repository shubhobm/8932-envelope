...
library(pracma)
library(psych)
setwd("C:/Study/UMN files/8932 envelope")
...
## Chemometrics data
data508 = read.csv("Book1.csv", header=T)
X = logt(as.matrix(data508[-c(1,2),-c(1,ncol(data508))]))
Y = as.matrix(data508[-c(1,2),ncol(data508)])
vartype = as.factor(as.matrix(data508[1,-c(1,ncol(data508))]))
label = as.matrix(data508[2,-c(1,ncol(data508))])
...
supSVD.analyze(X=scale(Xtc), Y=scale(Y), r=5, quiet=T)
z = supSVD.analyze(X=scale(cbind(Xts,Xtc)), r=5, Y=scale(Y), quiet=T); z$TruthTable
z = supSVD.analyze(X=scale(X), Y=scale(Y), r=10, maxit=100, quiet=T); z$TruthTable
