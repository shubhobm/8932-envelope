...
loglik = function(n,p,X,Y,B,V,Sf,s2){
  ...
  f2 = X - Y%*%B%*%t(V)
  return(-n*p*log(2*pi)/2 - n*sum(log(eigen(f1)$values))/2
         - tr(f2%*%f1%*%t(f2))/2)
}
...
supSVD.analyze = function(X, Y, r=NULL, tol=1e-5, maxit=1e3, quiet=T){
  
  # PCA of data matrix
  s2 = svd(X)$d; ss2 = sum(s2)
  r = ifelse(is.null(r), min(which(cumsum(s2)>=0.90*ss2)), r)
  s2.sel = s2[1:r]
