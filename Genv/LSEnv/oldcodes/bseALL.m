function [bseOLS,bsePLS,bseCCA,bseXenv,bseYenv,bseSenv] = bseALL(datavec,p,r,l,m,N,nsim,dPLS,dCCA);
mu = mean(datavec);
...
bseOLS = sqrt(diag(get_cov(beta_ols)));
bsePLS = sqrt(diag(get_cov(beta_pls)));
bseCCA = sqrt(diag(get_cov(beta_cca)));
bseXenv = sqrt(diag(get_cov(beta_Xenv)));
bseYenv = sqrt(diag(get_cov(beta_Yenv)));
bseSenv = sqrt(diag(get_cov(beta_env)));
