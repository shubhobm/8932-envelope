function f = PoissonCV_chisq_1DfromPLS(Y,X,u,nfold)
[N p] = size(X);
Ntest = floor(N/nfold);
X0 = X;
Y0 = Y;
...
%     [a_lik G_lik e_lik] = Poisson_ini(Y,X,s1,s2,s3);
    ...
    Err0 = Err0 + sum(((Ynew - p0).^2)./p0);
    Err_pls = Err_pls + sum(((Ynew - p_pls).^2)./p_pls);
    Err_mani = Err_mani + sum(((Ynew - p_mani).^2)./p_mani);
    Err_lik = Err_lik + sum(((Ynew - p_lik).^2)./p_lik);
    Err_lik1D = Err_lik1D + sum(((Ynew - p_lik1D).^2)./p_lik1D);;
end
