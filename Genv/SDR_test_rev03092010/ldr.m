...
%    d: dimension of estimated central subspace. (This is only useful when estimating
...
%    method: model used for updating. So far, accepted models are:
...
%            'SPFC': structured principal components (Cook and Forzani,2008-a)
%            'CORE': covariance reduction (Cook and Forzani, 2008-b)
%            'LAD': likelihood acquired directions (Cook and Forzani, 2008-c)
%
...
%    varargin: group of optional arguments. They can be set in hardly any order, 
%              just with the last one reserved to '-v' for enabling verbose option.
%              They must be given as a string-numeric pair, with the string telling 
%              about the option and the following scalar setting the value to be 
%              asigned to it. Available options depends on the selected model:
%              - LAD: 
...
%              'npermute': to set the number of samples for permutation
%              tests. Default value is npermute=500.
%              'initval': to set an initial estimate to start optimization.
%              If no such value is given, an initial estimate is guessed
